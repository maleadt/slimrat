...
# Package definition
package Slimrat::Server::Configuration;

=pod

=head1 NAME

Slimrat::Server::Configuration - Slimrat serverside configuration handler

=head1 DESCRIPTION

The C<Slimrat::Server::Config> package contains functionality to
configure the static aspects of slimrat. It reads structured key/value data
from a certain (local) file, and presents those values in a hash-like way
to be used by slimrat's subsystems.
...
# Packages
use Moose;
use Slimrat::Server::Configuration::Entry;
...
has 'items' => (
	is	=> 'ro',
	isa	=> 'HashRef[Slimrat::Server::Configuration::Entry]',
	default	=> sub { {} }
);
...
sub get_section {
	...
	# Extract subsection
	my $configsection = new Slimrat::Server::Configuration(
		parent	=> $self,
		section	=> $section
	);
